...
const MyPage = ({userInfo, setUserInfo}) => {
    ...
    useEffect(() => {
        axios.get("/api/auth").then((res) => {
            ...
            if (data.first && location.state?.first === undefined && data.profile === "default") {
                navigate("/profile");
            }
        });
    ...
    const deletePlayList = () => {
        let sel = playlistBundle.find((i) => i.key === localStorage.getItem("playlistKey"));
        if (sel.clientId === userInfo.id) {
            axios.post(`/api/playlist/delete/${localStorage.getItem("playlistKey")}`, {
            ...
            axios.post(`/api/playlist/remove/${localStorage.getItem("playlistKey")}`, {
                clientId: userInfo.id,
            }).then((res) => {
    ...
    return (
        <>
            <S.Container>
                ...
                {plusModalBool && (
                    <Modal leftText="취소" leftEvent={() => setPlusModalBool(false)}
                           rightText="생성" rightEvent={postAppendPlaylist}>
                        <S.ModalTitle>재생목록 생성</S.ModalTitle>
                        <S.IntroduceText>생성할 재생목록의 이름을 입력해 주세요.</S.IntroduceText>
                        <S.NameInput onChange={onChangePlaylistName} value={playlistName}
                ...
                {deleteModalBool && (
                    <Modal leftText="취소" leftEvent={() => setDeleteModalBool(false)}
                           rightText="삭제" rightEvent={deletePlayList}>
                        <S.ModalTextLayout>
                ...
                <S.InfoLayout>
                    ...
                    <PlaylistSection setPlusModalBool={setPlusModalBool} playlistBundle={playlistBundle}
                                     setDeleteModalBool={setDeleteModalBool} userInfo={userInfo}/>
                </S.InfoLayout>
                <Ellipse/>
            </S.Container>
            <Footer/>
